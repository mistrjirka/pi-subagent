...
13. `agent_wait` timeout snapshots and `agent_inspect` share one parent-side transcript cursor backed by Pi RPC `get_entries(since)`. Each successful read returns the next bounded page of previously unread child messages; messages already returned by either tool are not repeated, unread messages beyond the page remain queued, and an RPC failure never advances the cursor. `agent_inspect` remains read-only with respect to the child. Plaintext thinking is included when available; thinking expansion is additive and must not reduce the selected tool/message evidence.
